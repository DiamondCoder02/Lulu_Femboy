	...
	execute(message, client) {
		...
		if (message.channel.type === ChannelType.DM || client.settings.get(message.guild.id, "enableRandomReactions")) {
			...
			if (canSend) {
				...
				if (message.content.toLowerCase().includes("good bot")) { message.channel.send("Thank you! :3") }
				if (message.content.toLowerCase().includes("bad bot")) { message.channel.send("I'm sorry. >.<") }
